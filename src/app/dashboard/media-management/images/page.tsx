...
export default function ImagesPage() {
  ...
  // Ensure mediaItems is always an array and filter for images only
  const safeMediaItems: MediaItem[] = Array.isArray(mediaItems) ? mediaItems : []
  const imageItems = safeMediaItems.filter(item => 
  ...
  const syncedImages = imageItems.filter(item => item.syncedToWebsite)
  ...
  const syncMutation = useMutation({
    ...
    onSuccess: (_, action) => {
      toast.success(action === 'sync' 
        ? 'Image synced to website successfully!' 
        : 'Image removed from website successfully!'
      )
